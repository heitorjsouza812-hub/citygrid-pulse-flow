...
export function CardZona({ zona, compact = false }: { zona: Zona; compact?: boolean }) {
  ...
  const railColor = riscoColor(zona.risco_xgb);
    ...
    >
      ...
      {!compact && (
        <div className="mt-3 flex items-end justify-between gap-2">
          <div className="min-w-0">
            <div className="text-[9px] uppercase tracking-[0.22em] text-muted-foreground font-mono font-bold mb-1">Prev. 30min</div>
          ...
          <div className="flex flex-col items-end gap-0.5 text-[10px] font-mono shrink-0">
            ...
            <span className="text-muted-foreground text-[9px] uppercase tracking-wider">{zona.bat_modo}</span>
          </div>
        </div>
      )}
...
function Stat({ label, val, unit }: { label: string; val: string; unit: string }) {
  return (
    <div className="rounded-md border border-border/60 bg-surface/30 py-1.5 px-2">
      <div className="text-[8px] uppercase tracking-[0.18em] text-muted-foreground font-mono">{label}</div>
      <div className="font-mono text-sm font-bold text-foreground tabular-nums leading-tight mt-0.5">
        {val}<span className="text-[10px] text-muted-foreground ml-0.5 font-normal">{unit}</span>
      </div>
    </div>
  );
}
